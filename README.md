# ExtraTrees MT5 strategy files

## Files
- `train_mt5_extratrees_classifier.py`
- `MT5_ExtraTrees_Classifier_ONNX_Strategy.mq5`

## Python Installation
```powershell
pip install MetaTrader5 pandas numpy scikit-learn skl2onnx onnx
```

## Running Example
```powershell
python train_mt5_extratrees_classifier.py --symbol XAGUSD --timeframe M15 --bars 20000 --horizon-bars 8 --train-ratio 0.70 --output-dir output_et_XAGUSD_M15_h8
```

## Steps for MT5
1. Copy `ml_strategy_classifier_extratrees.onnx` file near the `.mq5` file
2. Recompile the EA
3. Run the tester only on the `TEST UTC` from `run_in_mt5.txt`

## Recommended EA start settings
- `InpUseTrendFilter = true`
- `InpTrendMAPeriod = 100`
- `InpUseTrendDistanceFilter = false`
- `InpUseAtrVolFilter = true`
- `InpAtrMinPercentile = 0.25`
- `InpAtrMaxPercentile = 0.85`
- `InpUseKillSwitch = false`
