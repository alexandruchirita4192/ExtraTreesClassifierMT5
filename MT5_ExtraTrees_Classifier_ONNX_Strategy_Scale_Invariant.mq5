...
#property description "EA MT5: ExtraTrees classifier ONNX, run in Strategy Tester"
#property description "With trend filter + ATR volatility filter + kill switch optional; ONNX features use ATR percent for scale invariance"

#include <Trade/Trade.mqh>

#resource "ml_strategy_classifier_extratrees.onnx" as uchar ExtModel[]
   ...
   if(signal == SIGNAL_BUY)
     {
      ...
      ok = trade.Buy(InpLots, _Symbol, ask, sl, tp, "ExtraTrees buy");
      if(ok)
         g_bars_in_trade = 0;
     }
   else if(signal == SIGNAL_SELL)
     {
      ...
      ok = trade.Sell(InpLots, _Symbol, bid, sl, tp, "ExtraTrees sell");
      if(ok)
         g_bars_in_trade = 0;
     }
  }
